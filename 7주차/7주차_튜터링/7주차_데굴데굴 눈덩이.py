...
# 2. 해당 문제에서 최댓값이 나오는 경로가 여러개일 경우가 있는데 예를 들어 입력예시에서도 9->11->15(이동횟수:3), 4->5->11->15(이동횟수:4) 두 가지의 경우 눈덩이의 최댓값이 35로 같은데 답이 4인 것을 보아 눈덩이가 최댓값이고 이동 횟수가 최대인 값을 출력하라는 표현이 없어 을 명확히애매하다고 생각합니다! 눈덩이의 최댓값이 여러개일때 최대 이동 횟수를 출력하라는 조건이 추가되어야하지 않을까요?